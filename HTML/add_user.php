...
<html lang="en">
  ...
  <?php
    ...
    <script>
      ...
      var x = document.forms["addcheck"]["email"].value;
      <?php 
        while(isset($row['id'])){ ?>
          var x_db="<?php echo $row['email']; ?>"
          if (x == x_db) {
            alert('Email já registado. Insira um diferente para continuar.');
            return false;
            }
            <?php
            $row = pg_fetch_assoc($result);
  ...
  <form name="addcheck" action="add_user_action.php" role="form" class="form-horizontal" method="POST" onsubmit="return validateForm()">
                <div class="form-body" style="margin: 50px;">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-lg-10">
                          <input type="checkbox" name="is_admin" value="Yes" class="form-control">
                        </div>
                    </div>
...
<?php $_SESSION['previous_location'] = "add_user.php"; ?>
</html>