...
<nav class="navbar navbar-expand-lg navbar-light bg-light";>
        <ul class="navbar-nav ml-auto">
            ...
            <li class="nav-item">
                <a href="admin_reparacoes.php" class="nav-link">Reparações</a>
            </li>
        </ul>
</nav>

<div class="container my-4">
<h3> Consultar users</h3>
...
<table class="table table-striped">
  ...
  <?php while(isset($row['id'])){ ?>
    <tr>
      ...
      <td><?php if ($row['is_admin']=='t'){echo "Sim";} else{echo "Não";} ?></td>
      <td>
        <form action="edit_remove_user_action.php" method="POST">
        <button type="submit" name="editar" value="<?php echo $row['id']; ?>"class="btn btn-warning"><i class="fa fa-pencil"></i> Edit</button>
        <button type="submit" name="remover" value="<?php echo $row['id']; ?>" class="btn btn-danger"><i class="fa fa-remove"></i> Remove</button>
        </form>
      </td>
    </tr>
  <?php
...
<a href="add_user.php"><button type="submit" action="infopessoal.php" class="btn btn-success"><i class="fa fa-plus"></i> Adicionar</button></a>
